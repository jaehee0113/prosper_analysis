...
ggplot(pd, aes((InvestmentFromFriendsCount / Investors))) +
  geom_bar(fill = "dodgerblue", colour = "gray48") +
  labs(x = 'Friends and Investors Ratio', y = 'Frequency') +
...
br_state_dist = ggplot(aes(BorrowerState), 
  ...
  geom_hline(yintercept = mean(pd.pd_by_states$n), alpha =  0.3, linetype = 2, size = 1, color = I('red')) +
...
render_graph(pd,'Frequency of borrower being a home owner', 'IsBorrowerHomeowner') +
  get_correct_geom('bar') +
  labs(x = 'Home Owner', y = 'Frequency')
```

We can see that the fact that you are a home owner has no correlation with becoming a borrower.

```{r echo=FALSE, message=FALSE, warning=FALSE, income_dist, fig.width = 12}
render_graph(pd,'Distribution of borrowers` income range','IncomeRange') +
  get_correct_geom('bar') +
...
render_graph(pd.pd_by_listing_categories,'Distribution of listings by category','ListingCategory..numeric.', 'n') +
  geom_point() +
...
group_loss <- pd %>%
  ...
  summarise_each(funs(mean(., na.rm = TRUE)), EstimatedLoss)

group_loss_plot <- ggplot(subset(group_loss, EstimatedLoss != 'NaN')) +
  geom_bar(aes(x = EmploymentStatus, y = EstimatedLoss),
           stat = "identity",
           na.rm = TRUE,
           fill = "dodgerblue",
           color = 'gray48') +
...
group_return <- pd %>%
  group_by(EmploymentStatus) %>%
  summarise_each(funs(mean(., na.rm = TRUE)), EstimatedReturn)

group_return_plot <- ggplot(subset(group_return, EstimatedReturn != 'NaN')) +
  geom_bar(aes(x = EmploymentStatus, y = EstimatedReturn),
           stat = "identity",
           na.rm = TRUE,
           fill = "dodgerblue",
           color = 'gray48') +
...
```{r echo=FALSE, message=FALSE, warning=FALSE,emp_status}
group_yield <- pd %>%
  group_by(EmploymentStatus) %>%
  summarise_each(funs(mean(., na.rm = TRUE)), EstimatedEffectiveYield)

group_yield_plot <- ggplot(subset(group_yield, EstimatedEffectiveYield != 'NaN')) +
  geom_bar(aes(x = EmploymentStatus, y = EstimatedEffectiveYield),
           stat = "identity",
           na.rm = TRUE,
           fill = "dodgerblue",
           color = 'gray48') +
  ggtitle('Estimated effective yield rate mean for each employment status') +
...
ggplot(subset(group_funded, 
                     (Occupation %in% c('Computer Programmer', 'Analyst', 'Executive', 'Attorney', 'Doctor', 'Judge', 'Professor')))) +
...
### Multivariate Analysis (Final Plots)

As a summary plot, we have seen how income range is correlated with the probability of one borrowing money using this platform. Therefore, it would be good to know: for each income range, how loan status and employment status are related together (interrelationship among the variables). I discarded 'Not displayed' and 'Not employed' in the facetting process because they are redundant for our purpose. Although $0 may be data to be cleaned, I included it in because it may reveal some useful insight to our analysis. These are the final plots and summarize my findings (the most important findings are sort of merged in this multivariate analysis.)
